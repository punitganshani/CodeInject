...
{
    internal class MonoAssemblyResolver : BaseAssemblyResolver
    {
        ...
        internal List<TypeDefinition> FindTypes<T1>() where T1 : ICInject
        {
        ...
        internal List<TypeDefinition> FindClasses()
        {
            return _assembly.MainModule.Types.Where(x => x.IsClass).ToList();
        }

        internal bool Inject(Type type)
        {
        ...
        internal bool Inject(MethodDefinition methodDefinition, Type type)
        {
            bool success = true;
            if (Assembly.MainModule.GetRuntime() != type.Assembly.GetRuntime())
            {
